...
class QLearnTicTac(BaseReward):
    ...
    def center_move(self, board, move, first_move):
        reward = 0
        size = len(board)
        center = size // 2
    ...
    def block_win(self, player_id, all_lines, move, win_length):
        ...
        for line in all_lines:
            ...
            ## Might need adjustment for larger boards
    ...
    def possible_win_lines(self, player_id, all_lines, move, win_length):
        all_lines
        reward = 0
        ...
        for line in all_lines:
            ...
            if any(r == move.target_row and c == move.target_col for (r, c, _) in line):
                if player_count + empty_count >= win_length:
                    reward += 0.05
    ...
    def compute_reward(self, player_id, winner, draw, board_state, move, config):
        ...
        first_move = self.is_first_move(board_state)

        if first_move:
            reward += self.center_move(board_state, move, first_move)
